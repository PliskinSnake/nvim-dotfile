...
return {
  ...
  config = function()
    ...
    vim.keymap.set("n", "<leader>Jt", dap.toggle_breakpoint, { desc = "[D]ebug [T]oggle Breakpoint" })

    -- set a vim motion for <Space> + d + s to start the debugger and launch the debugging ui
    vim.keymap.set("n", "<leader>Js", dap.continue, { desc = "[D]ebug [S]tart" })

    -- set a vim motion to close the debugging ui
    vim.keymap.set("n", "<leader>Jc", dapui.close, { desc = "[D]ebug [C]lose" })
  end,
}
